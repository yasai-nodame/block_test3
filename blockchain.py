...
class BlockChain(object):
    ...
    def create_new_block(self, creator):
        ...
        while not self.hash(block_without_time)[:PROOF_OF_WORK_DIFFICULTY] == '0'*PROOF_OF_WORK_DIFFICULTY: #hash値のスライス4番目までが0000ではない間while文を回す
        ...
        #新ブロックに含まれているトランザクション情報のみをトランザクションプールから消去
        #block["transactions"]はself.transaction_pool["transactions"].copy()なので実行中に追加されても一致はしない
    ...
    def verify_transaction(self, transaction):
        public_key = VerifyingKey.from_string(binascii.unhexlify(transaction.sender), curve=SECP256k1) #16進数文字列をバイト列に、バイト列をKeyオブジェクトに変換　senderの公開鍵
        signature = binascii.unhexlify(transaction.signature) #16進数文字列からバイト列に変換　署名データ
        ...
        return public_key.verify(signature, transaction_unsigned_bytes) #バイト型にした署名データとバイト型にした生のデータを検証する
